    ...
    match split_bytes_at(encoded_value, ':') {
        Some((number_string, string)) => {
            let number = number_string
                ...
                .unwrap();
            (
                String::from_utf8_lossy(&string[..number as usize]).into(),
                &string[number as usize..],
            )
        }
...
mod tests {
    ...
    fn test_decode_bencoded_list() {
        assert_eq!(
            decode_bencoded_value("l5:helloi52ee".as_bytes()),
            json!(["hello", 52])
        );
    }
    ...
    fn test_decode_simple_dict() {
        assert_eq!(
            decode_bencoded_value("d3:foo3:bar5:helloi52ee".as_bytes()),
